  ...
  return (
    <>
      ...
      <ResponsiveContainer className={styles["piechart-container"]} width="100%" height="100%">
        <PieChart>
          <Pie data={data} innerRadius={70} outerRadius={80} dataKey="score">
            {data.map((entry, index) => (
              <Cell key={`cell-${index}`} fill={index === 0 ? "#ff0000" : "#fbfbfb"} stroke={index === 0 ? null : "#fbfbfb"} cornerRadius={index === 0 ? 10 : null} />
            ))}
          </Pie>
        </PieChart>
      </ResponsiveContainer>
